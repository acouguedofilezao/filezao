...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='SilentlyContinue'; $repo='%cd%'; $dirs=@($repo,(Join-Path $env:USERPROFILE 'Downloads')); Add-Type -AssemblyName System.IO.Compression.FileSystem; $zips=@(); foreach($d in $dirs){ if(Test-Path $d){ $zips += Get-ChildItem -LiteralPath $d -Filter *.zip -File } }; $zips=$zips | Sort-Object LastWriteTime -Descending; $sig=@('index.html','tv.html','CHANGELOG.md','anuncio.mp3','anuncio2.mp3'); $zip=$null; foreach($z in $zips){ try { $a=[System.IO.Compression.ZipFile]::OpenRead($z.FullName); $has=($a.Entries | Where-Object { $sig -contains $_.Name } | Select-Object -First 1); $a.Dispose(); if($has){ $zip=$z; break } } catch {} }; if(-not $zip){ Write-Host '  Nenhum .zip do sistema encontrado - vou usar o que ja esta na pasta.'; exit }; Write-Host ('  Usando: ' + $zip.Name + '   (em: ' + $zip.DirectoryName + ')'); $tmp=Join-Path $env:TEMP 'filezao_unzip'; if(Test-Path $tmp){ Remove-Item $tmp -Recurse -Force }; New-Item -ItemType Directory -Path $tmp | Out-Null; try { Expand-Archive -LiteralPath $zip.FullName -DestinationPath $tmp -Force } catch { Write-Host ('  [aviso] falha ao extrair: ' + $_.Exception.Message); exit }; $n=0; Get-ChildItem -Path $tmp -Recurse -File -Include *.html,*.md,*.mp3 | ForEach-Object { $dest=Join-Path $repo $_.Name; Copy-Item $_.FullName -Destination $dest -Force; (Get-Item $dest).LastWriteTime=Get-Date; Write-Host ('  atualizado: ' + $_.Name); $n++ }; Remove-Item $tmp -Recurse -Force; Write-Host ('  ' + $n + ' arquivo(s) trazido(s) do .zip.') "
echo.

echo Arquivos nesta pasta - data e hora da ultima alteracao:
for %%f in (index.html tv.html CHANGELOG.md anuncio2.mp3) do call :MOSTRA "%%f"
echo.
