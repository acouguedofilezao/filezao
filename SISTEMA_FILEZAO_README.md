...
- **Banco de dados (Supabase):** projeto `vfrgqtuvbflkexapdzho` → URL `https://vfrgqtuvbflkexapdzho.supabase.co`
  ...
  - **Tabelas:** `entradas`, `saidas`, `gado`, `cheques`, `produtos`, `energia`, `saldos`, `saldos_dia`, `cotacoes`, `logs`, `folha`, **`config`** (chave/valor: `meta_semanal`, `bonus`, `anotacoes`, `escala` do painel; e `permissoes` do controle de acesso). (Os funcionários ficam só no aparelho, na chave `fz_func`.)
